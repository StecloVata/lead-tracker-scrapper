...
export default function OnboardingModal({ userId }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[9990] flex items-center justify-center p-4" style={{ background: "rgba(15,20,50,0.55)", backdropFilter: "blur(4px)" }}>
      {screen === "leads" ? (
        <div className="w-full max-w-lg rounded-2xl overflow-hidden" style={{ background: "var(--card)", boxShadow: "var(--shadow-lg)" }}>
          {/* Header */}
          <div className="px-8 pt-9 pb-3 text-center">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-5" style={{ background: "var(--navy)" }}>
            ...
            <h2 className="text-xl font-bold mb-1.5 tracking-tight" style={{ color: "var(--text)" }}>Welcome to Adversus Leads</h2>
          ...
          <div className="px-8 py-6 space-y-3">
            ...
            <button
              ...
              style={{
                borderColor: seedChoice === "seed" ? "var(--primary)" : "var(--border)",
                background: seedChoice === "seed" ? "var(--primary-soft)" : "var(--card)",
                boxShadow: seedChoice === "seed" ? "0 0 0 3px var(--ring)" : "none",
              }}
            >
            ...
            <button
              ...
              className="w-full text-left p-4 rounded-xl border-2 transition-all"
              style={{
                borderColor: seedChoice === "empty" ? "var(--primary)" : "var(--border)",
                background: seedChoice === "empty" ? "var(--primary-soft)" : "var(--card)",
                boxShadow: seedChoice === "empty" ? "0 0 0 3px var(--ring)" : "none",
              }}
            >
              <div className="flex items-start gap-3">
                <span className="text-2xl mt-0.5">✨</span>
                <div>
                  ...
                  <span className="inline-block mt-2 text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: "var(--card-alt)", color: "var(--text-sub)" }}>
                    Clean slate
                  </span>
                </div>
              </div>
        ...
        <div className="w-full max-w-lg rounded-2xl overflow-hidden" style={{ background: "var(--card)", boxShadow: "var(--shadow-lg)" }}>
          <div className="px-8 pt-9 pb-3 text-center">
            ...
            <h2 className="text-xl font-bold mb-1.5 tracking-tight" style={{ color: "var(--text)" }}>Take the tour?</h2>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-sm flex items-center gap-2" style={{ color: "var(--text)" }}>
                    Yes, show me around
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: "var(--primary)", color: "#fff" }}>Recommended</span>
                  </div>
                  <div className="text-xs mt-1.5 leading-relaxed" style={{ color: "var(--muted)" }}>
